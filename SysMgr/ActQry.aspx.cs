...
public partial class SysMgr_ActQry :  BasePage 
{
    ...
    private DataTable GetDataTable()
    {
        //讀取資料庫
        string strSql = @"
                    SELECT  a.uid as uid,ClassName as 類別, itemname as 項目,Remark as 說明,CONVERT( NVARCHAR(12),BegDate,111 )  as 活動日期,
                    CONVERT( NVARCHAR(12),EndDate,111 ) as 報名結束日期 ,
                    (Places  - (SELECT COUNT(*) FROM  ActMap WHERE ActUid =a.uid and  ISNULL(IsDelete ,'') != 'Y' )) AS 尚餘名額,
                    itemuid,a.classuid as classuid
