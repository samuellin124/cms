...
public partial class CaseMgr_ServiceClassQry : BasePage
{
    ...
    private void LoadFormData()
    {
        ...
        npoGridView.EditLink = Util.RedirectByTime("ServiceClassEdit.aspx", "ServiceClassUID=");
        lblGridList.Text = npoGridView.Render();
    }
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        Response.Redirect(Util.RedirectByTime("ServiceClassEdit.aspx?Mode=ADD&"));
    }
    ...
    private DataTable GetDataTable()
    {
        //讀取資料庫
        string strSql = @"
                        select uid,ClassName as 服務類別 from serviceclass
                        where 1=1 
                        and isnull(IsDelete, '') != 'Y'
                        ";
        if (txtName.Text != "")
        {
            strSql += " and ClassName like @ClassName\n";
        }
        //if (txtContact.Text != "")
        //{
        ...
        dict.Add("ClassName", "%" + txtName.Text + "%");
