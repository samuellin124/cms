...
public partial class CaseMgr_ServDataEdit : BasePage
{
    GroupAuthrity ga = null;
    protected void Page_Load(object sender, EventArgs e)
    {
        Session["ProID"] = "ServData";
        //權限控制
        AuthrityControl();
        ...
        if (!IsPostBack)
        {
            HFD_ServDataUID.Value = Util.GetQueryString("ServDataUID");
   ...
   private void LoadFormData()
    {
        ...
        if (HFD_Mode.Value != "ADD")
        {
            ...
            strSql = @"
                        select NameUID ,ServiceDate ,Administrative ,
                        Event , VolunteerCadre 
                        from ServData
                        where uid=@uid
                      ";
            ...
            dict.Add("uid", HFD_ServDataUID.Value);
            dt = NpoDB.GetDataTableS(strSql, dict);
            if (dt.Rows.Count == 0)
            {
                //資料異常
                //SetSysMsg("資料異常!");
                Response.Redirect("ServDataQry.aspx");
                return;
            }
            dr = dt.Rows[0];
       ...
       //志工編號
        txtCName.Text = (HFD_Mode.Value == "ADD") ? "" : dr["NameUID"].ToString();
       //服務日期
         //txtServDate.Text = (HFD_Mode.Value == "ADD") ? "" : dr["ServiceDate"].ToString();
         txtServDate.Text = (HFD_Mode.Value == "ADD") ? "" : Util.DateTime2String(dr["ServiceDate"], DateType.yyyyMMdd, EmptyType.ReturnEmpty);
        //判斷[行政服務]的資料庫是否空值在讀取資料
        if (HFD_Mode.Value != "ADD" && dr["Administrative"].ToString() != "")
        {
            string Administrative = dr["Administrative"].ToString();
            string[] split = Administrative.Split(new Char[] { ',' });
            split[0].ToString();
            split[1].ToString();
            if (HFD_Mode.Value != "ADD")
                CK_Administrative1.Checked = split[0].ToString() == "接聽真情之夜" ? true : false;
            if (HFD_Mode.Value != "ADD")
                CK_Administrative2.Checked = split[1].ToString() == "資料整合鍵入" ? true : false;
        }
        if (HFD_Mode.Value != "ADD" && dr["Event"].ToString() != "")
        {
            string Event = dr["Event"].ToString();
            string[] split = Event.Split(new Char[] { ',' });
            split[0].ToString();
            split[1].ToString();
            split[2].ToString();
            split[3].ToString();
            if (HFD_Mode.Value != "ADD")
                CK_Event1.Checked = split[0].ToString() == "讀書會" ? true : false;
            if (HFD_Mode.Value != "ADD")
                CK_Event2.Checked = split[1].ToString() == "在職教育" ? true : false;
            if (HFD_Mode.Value != "ADD")
                CK_Event3.Checked = split[2].ToString() == "培訓服務" ? true : false;
            if (HFD_Mode.Value != "ADD")
                CK_Event4.Checked = split[3].ToString() == "參與活動" ? true : false;
        }
        if (HFD_Mode.Value != "ADD" && dr["VolunteerCadre"].ToString() != "")
        {
            string VolunteerCadre = dr["VolunteerCadre"].ToString();
            string[] split = VolunteerCadre.Split(new Char[] { ',' });
            split[0].ToString();
            split[1].ToString();
            split[2].ToString();

            if (HFD_Mode.Value != "ADD")
                CK_VolunteerCadre1.Checked = split[0].ToString() == "擔任組長" ? true : false;
            if (HFD_Mode.Value != "ADD")
                CK_VolunteerCadre2.Checked = split[1].ToString() == "讀書會帶領人" ? true : false;
            if (HFD_Mode.Value != "ADD")
                CK_VolunteerCadre3.Checked = split[2].ToString() == "擔任夜間掌門人" ? true : false;
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        SetColumnData(list);
        string strSql = Util.CreateInsertCommand("ServData", list, dict);
        NpoDB.ExecuteSQLS(strSql, dict);
        Session["Msg"] = "新增資料成功";
        Response.Redirect(Util.RedirectByTime("ServDataQry.aspx"));
    }
    //-------------------------------------------------------------------------------------------------------------
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        List<ColumnData> list = new List<ColumnData>();
        SetColumnData(list);
        string strSql = Util.CreateUpdateCommand("ServData", list, dict);
        NpoDB.ExecuteSQLS(strSql, dict);
        Session["Msg"] = "修改資料成功";
        Response.Redirect(Util.RedirectByTime("ServDataQry.aspx"));
    }
    ...
    {
        list.Add(new ColumnData("Uid", HFD_ServDataUID.Value, false, false, true));
        //志工編號
        list.Add(new ColumnData("NameUID", txtCName.Text, true, true, false));
        //服務日期
        list.Add(new ColumnData("ServiceDate", Util.DateTime2String(txtServDate.Text , DateType.yyyyMMdd, EmptyType.ReturnEmpty), true, true, false));
        //行政服務
        string Administrative = "";
        if (CK_Administrative1.Checked == true)
            Administrative = "接聽真情之夜,";
        else
            Administrative = ",";
        if (CK_Administrative2.Checked == true)
            Administrative += "資料整合鍵入,";
        else
            Administrative += ",";
        list.Add(new ColumnData("Administrative", Administrative, true, true, false));
        //活動服務
        string Event = "";
        if (CK_Event1.Checked == true)
            Event += "讀書會,";
        else
            Event += ",";
        if (CK_Event2.Checked == true)
            Event += "在職教育,";
        else
            Event += ",";
        if (CK_Event3.Checked == true)
            Event += "培訓服務,";
        else
            Event += ",";
        if (CK_Event4.Checked == true)
            Event += "參與活動,";
        else
            Event += ",";
        list.Add(new ColumnData("Event", Event, true, true, false));
        //志工幹部活動
        string VolunteerCadre = "";
        if (CK_VolunteerCadre1.Checked == true)
            VolunteerCadre += "擔任組長,";
        else
            VolunteerCadre += ",";
        if (CK_VolunteerCadre2.Checked == true)
            VolunteerCadre += "讀書會帶領人,";
        else
            VolunteerCadre += ",";
        if (CK_VolunteerCadre3.Checked == true)
            VolunteerCadre += "擔任夜間掌門人,";
        else
            VolunteerCadre += ",";
        list.Add(new ColumnData("VolunteerCadre", VolunteerCadre, true, true, false));
    ...
    {
        Response.Redirect("ServDataEdit.aspx");
    }
    //-------------------------------------------------------------------------------------------------------------
    //諮詢資料
