...
public partial class CaseMgr_ServiceItemQry : BasePage
{
    ...
    private void LoadFormData()
    {
        ...
        npoGridView.EditLink = Util.RedirectByTime("ServiceItemEdit.aspx", "ServiceItemUID=");
    ...
    private DataTable GetDataTable()
    {
        //讀取資料庫
        string strSql = @"
                        select b.uid as uid ,ClassName as 服務類別,ItemName as 服務項目, Score as 分數 from serviceclass a inner join serviceitem b on a.uid=b.classuid
                        where 1=1 
                        and isnull(b.IsDelete, '') != 'Y'
                        ";
        if (txtName.Text != "")
        {
            strSql += " and ItemName like @ItemName\n";
        }
        if (ddlclassname.SelectedValue != "")
        {
            strSql += " and classuid like @classuid\n";
        }
        //if (txtContact.Text != "")
        //{
        ...
        dict.Add("ItemName", "%" + txtName.Text + "%");
