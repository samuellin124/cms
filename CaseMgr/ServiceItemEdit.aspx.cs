...
public partial class CaseMgr_ServiceItemEdit : BasePage
{
    GroupAuthrity ga = null;
    protected void Page_Load(object sender, EventArgs e)
    {
        Session["ProID"] = "ServiceItem";
        //權限控制
        AuthrityControl();
        ...
        if (!IsPostBack)
        {
            HFD_ServiceItemUID.Value = Util.GetQueryString("ServiceItemUID");
   ...
   private void LoadFormData()
    {
        ...
        if (HFD_Mode.Value != "ADD")
        {
            ...
            strSql = @"
                        select a.uid  ,b.uid,ClassName as 服務類別,classuid,ItemName as 服務項目, Score as 分數 
                        from serviceclass a inner join serviceitem b on a.uid=b.classuid
            ...
            dict.Add("uid", HFD_ServiceItemUID.Value);
            dt = NpoDB.GetDataTableS(strSql, dict);
            if (dt.Rows.Count == 0)
            {
                //資料異常
                //SetSysMsg("資料異常!");
                Response.Redirect("ServiceItemQry.aspx");
                return;
            }
            dr = dt.Rows[0];
       ...
       //服務項目
        txtItemName.Text = (HFD_Mode.Value == "ADD") ? "" : dr["服務項目"].ToString();
        //服務類別
        ddlclassname.SelectedValue = (HFD_Mode.Value == "ADD") ? "" : dr["classuid"].ToString();
      //分數
        txtScore.Text = (HFD_Mode.Value == "ADD") ? "" : dr["分數"].ToString();
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        SetColumnData(list);
        string strSql = Util.CreateInsertCommand("ServiceItem", list, dict);
        NpoDB.ExecuteSQLS(strSql, dict);
        Session["Msg"] = "新增資料成功";
        Response.Redirect(Util.RedirectByTime("ServiceItemQry.aspx"));
    }
    //-------------------------------------------------------------------------------------------------------------
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        List<ColumnData> list = new List<ColumnData>();
        SetColumnData(list);
        string strSql = Util.CreateUpdateCommand("ServiceItem", list, dict);
        NpoDB.ExecuteSQLS(strSql, dict);
        Session["Msg"] = "修改資料成功";
        Response.Redirect(Util.RedirectByTime("ServiceItemQry.aspx"));
    }
    ...
    {
        list.Add(new ColumnData("Uid", HFD_ServiceItemUID.Value, false, false, true));
        //服務項目
        list.Add(new ColumnData("ItemName", txtItemName.Text, true, true, false));
        //服務類別
        list.Add(new ColumnData("classuid", ddlclassname.SelectedValue, true, true, false));
        //分數
        list.Add(new ColumnData("Score", txtScore.Text, true, true, false));
    }
    //-------------------------------------------------------------------------------------------------------------
